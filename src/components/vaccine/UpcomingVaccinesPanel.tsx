...
const UpcomingVaccinesPanel: React.FC<UpcomingVaccinesPanelProps> = ({ onSelectSchedule }) => {
  ...
  const urgentSchedules = [...overdueSchedules, ...upcomingSchedules].slice(0, 5);
              ...
              <p className="text-xs text-muted-foreground">
                {schedule.status === 'overdue' ? (
                  <span className="text-overdue font-medium">
                    Quá hạn {formatDistanceToNow(parseISO(schedule.scheduled_date), { 
                      locale: vi,
                      addSuffix: false 
                    })}
                  </span>
