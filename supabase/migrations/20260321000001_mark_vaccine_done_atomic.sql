-- Migration: Tạo hàm atomic mark_vaccine_done_atomic
-- Đảm bảo tính nhất quán dữ liệu khi xác nhận tiêm chủng
...
BEGIN
  ...
  -- Trigger trg_recalculate_on_completion sẽ tự động được kích hoạt
...
EXCEPTION
  WHEN OTHERS THEN
    -- PL/pgSQL tự động rollback nếu có ngoại lệ trong khối BEGIN...END
    RAISE;
END;
$$;
