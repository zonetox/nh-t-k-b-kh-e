...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session: existingSession } }) => {
      ...
      if (existingSession?.user) {
        fetchProfile(existingSession.user.id).then(data => setProfile(data as unknown as Profile));
      }
  ...
  const login = async (phone: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      // Rate limit check (use 0.0.0.0 as fallback since client can't get real IP)
      ...
      try {
        ...
        if (rateCheck && typeof rateCheck === 'object' && 'allowed' in (rateCheck as any)) {
          const check = rateCheck as { allowed: boolean; reason?: string; remaining_seconds?: number };
          if (!check.allowed) {
            const msg = check.reason === 'account_locked'
              ? `Tài khoản bị tạm khóa. Vui lòng thử lại sau ${Math.ceil((check.remaining_seconds || 60) / 60)} phút.`
      ...
      if (error) {
        ...
        try {
          await supabase.rpc('record_login_attempt', {
            p_phone: normalizedPhone,
            p_ip: ipAddress as unknown as string,
            p_success: false,
          });
        } catch (e) {
  ...
  const isPremium = profile?.is_premium || profile?.is_admin || false;
  ...
  const isTrialActive = !isPremium && trialDaysRemaining > 0;
